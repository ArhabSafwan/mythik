...
<h4 class="py-3 mb-4 fs-5 ">
    ...
    <span class="heading-color">All Modules </span>
</h4>
...
<div class="card">
    <div class="card-header">
        ...
        <a href="{{ route('modules.create') }}" style="color: white;">
            <button class="btn btn btn-primary waves-effect waves-light"  type="button">
                <span>
                    <i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>
                    <span class="d-none d-sm-inline-block">Create Module</span>
                </span>
            </button>
        </a>
    </div>
...
<script>
    $(document).ready(function() {
     ...
     var table = $('#usersTable').DataTable({
         ...
         responsive: true,
         ajax: {
             url: '{{ route("modules.index") }}',  // Your backend route to fetch user data
             data: function(d) {
 ...
 $(document).on('click', '.delete-btn', function() {
    var moduleID = $(this).data('id'); // Get the role ID
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '/admin/modules/' + moduleID + '/destroy',  // Route to the delete method
                type: 'DELETE',
                data: {
