...
<h4 class="py-3 mb-4 fs-5 ">
    ...
    <span class="heading-color">All Roles </span>
</h4>
        ...
        <a href="{{ route('roles.create') }}" style="color: white;">
            <button class="btn btn btn-primary waves-effect waves-light"  type="button">
                <span>
                    ...
                    <span class="d-none d-sm-inline-block">Create Role</span>
                </span>
            </button>
        </a>
    </div>
...
<script>
    $(document).ready(function() {
     ...
     var table = $('#usersTable').DataTable({
         ...
         responsive: true,
         ajax: {
             url: '{{ route("roles.index") }}',  // Your backend route to fetch user data
             data: function(d) {
 ...
 $(document).on('click', '.delete-btn', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '/admin/roles/' + roleId + '/destroy',  // Route to the delete method
                type: 'DELETE',
                data: {
