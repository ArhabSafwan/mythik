...
@section('main')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <h5 class="card-title mb-0">Field Visit Participants</h5>
        ...
        <div class="card-datatable text-nowrap p-4">
            <div class="table-responsive">
                <table class="table datatable-field-visits" id="fieldVisitTable">
                    <thead>
                        <tr>
...
@push('script')
    <script>
        $(document).ready(function() {
            var table = $('#fieldVisitTable').DataTable({
                processing: true,
                serverSide: true,
                        ...
                        action: function() {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    let url = "{{ route('field-visits.export') }}" +
                    ...
                    },
                    {
                        ...
                        action: function() {
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        url: "{{ route('field-visits.bulkStatusUpdate') }}",
                                        method: "POST",
                                        data: {
                                            _token: "{{ csrf_token() }}",
                                            ids: selected,
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                $('#fieldVisitTable').DataTable().ajax.reload(null,
                                    false);
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Deleted!',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                $('#fieldVisitTable').DataTable().ajax.reload(null, false);
                                Swal.fire({
                                    icon: 'success',
